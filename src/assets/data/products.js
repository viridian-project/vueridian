export default [{
    docType: "product",
    id: "418ff85c-ae31-448f-aebd-d77dd966a514",
    createdBy: "zecourlis",
    createdAt: "2016-02-10 16:58:18Z",
    status: "ACTIVE",
    updatedBy: "",
    updatedAt: "",
    supersedes: "",
    supersededBy: "",
    changeReason: "",
    score: {
      environment: -55,
      climate: -71,
      society: -7,
      health: -78,
      animalWelfare: 6,
      economy: 4
    },
    gtin: "7612100055557",
    producers: ["afd05a40-4ed6-4ae5-8120-eb7daebc336c"], // AnyCompany
    containedProducts: [],
    labels: ["42c2f586-a893-485f-8995-8639446bb6b8"], // UTZ
    locales: {
      en: {
        name: "The REAL Chocolate Crunchy Cream",
        price: "4.99",
        currency: "EUR",
        description: "Spread made from instant cocoa drink powder with added vitamins",
        quantities: ["400 g"],
        ingredients: "33% instant cocoa drink powder with added vitamins (sugar, cocoa powder, milk powder, vitamins (A, E, B1, B2, C), vanilla extract, salt), sugar, vegetable oil (colza, palm), low fat cocoa, emulgator sun flower lecithin, vanillin.",
        packagings: ["glass", "plastic"],
        categories: ["Spread", "Breakfast", "Chocolate cream"],
        imageUrls: ["/img/photos/crumpy_mod-140420_1.png"],
        urls: ["http://www.therealcream.com/"]
      }
    }
  },
  {
    docType: "product",
    id: "8fe77cbb-dddb-486a-b4a6-5680daa839d3",
    createdBy: "openfoodfacts-contributors",
    createdAt: "2018-12-24 12:11:54Z",
    status: "ACTIVE",
    updatedBy: "user123",
    updatedAt: "2019-02-10 18:33:39Z",
    supersedes: "product-bc36d43e-c40c-40c0-8086-dc19bc000fe1",
    supersededBy: "",
    changeReason: "Wrong quantity information.",
    score: {
      environment: 40,
      climate: -20,
      society: -25,
      health: 0,
      animalWelfare: 15,
      economy: 10
    },
    gtin: "7612100055557",
    producers: ["afd05a40-4ed6-4ae5-8120-eb7daebc336c"], // AnyCompany
    containedProducts: [],
    labels: ["e867d2ff-940b-4ee0-b063-11c0812b87a6"], // EU organic
    locales: {
      en: {
        name: "GreenLine Organic Chocolate Spread",
        price: "5.99",
        currency: "EUR",
        description: "Organic chocolate spread containing genuine vanilla",
        quantities: ["400 g"],
        ingredients: "33% instant cocoa drink powder with added vitamins (sugar, cocoa powder, milk powder, vitamins (A, E, B1, B2, C), vanilla extract, salt), sugar, vegetable oil (colza, palm), low fat cocoa, emulgator sun flower lecithin, vanillin.",
        packagings: ["glass"],
        categories: ["Spread", "Breakfast", "Chocolate cream"],
        imageUrls: ["/img/photos/crumpy_mod-160420_2.2.png"],
        urls: ["http://www.greenline-organic.com/chocolate-spread/"]
      }
    }
  },
  {
    docType: "product",
    id: "1433eb64-58c5-4fa3-93cb-c46f132dfca6",
    createdBy: "ElisabethKent2015",
    createdAt: "2020-01-29 10:44:11Z",
    status: "ACTIVE",
    updatedBy: "",
    updatedAt: "",
    supersedes: "",
    supersededBy: "",
    changeReason: "",
    score: {
      environment: 67,
      climate: 50,
      society: 80,
      health: 20,
      animalWelfare: 30,
      economy: 35
    },
    gtin: "7142985939201",
    producers: ["8396a807-4082-4e11-a003-7151f545d0c4"], // Willow Organic Foods, Inc.
    containedProducts: [],
    labels: [
      "e867d2ff-940b-4ee0-b063-11c0812b87a6", // EU organic
      "4c162084-b1a5-453e-86ab-bcf5cc6fc8e7", // Bioland
      "bd32b588-07e4-4292-a5c4-db05a56ab94c" // Fairtrade
    ],
    locales: {
      en: {
        name: "Willow organic creamy chocolate spread",
        price: "5.99",
        currency: "EUR",
        description: "Organic chocolate spread, palm oil free",
        quantities: ["300 g"],
        ingredients: "33% raw cocoa powder, vegetable oil (colza, coconut), sugar, vanilla extract, salt, emulgator sun flower lecithin.",
        packagings: ["glass", "metal"],
        categories: ["Spread", "Breakfast", "Chocolate cream"],
        imageUrls: ["images/Willow_organic_creamy_chocolate_spread.jpg"],
        urls: ["https://willoworganic.com/products/creamy/"]
      }
    }
  },
  {
    docType: "product",
    id: "e01f907e-60f4-4ef8-9fa2-92b36508ebde",
    createdBy: "londonveganista",
    createdAt: "2020-05-01 11:01:02Z",
    status: "ACTIVE",
    updatedBy: "",
    updatedAt: "",
    supersedes: "",
    supersededBy: "",
    changeReason: "",
    score: {
      environment: 80,
      climate: 60,
      society: 40,
      health: 80,
      animalWelfare: 80,
      economy: 35
    },
    gtin: "7998292838391",
    producers: ["8396a807-4082-4e11-a003-7151f545d0c4"], // Willow Organic Foods, Inc.
    containedProducts: [],
    labels: [
      "e867d2ff-940b-4ee0-b063-11c0812b87a6", // EU organic
      "4c162084-b1a5-453e-86ab-bcf5cc6fc8e7", // Bioland
      "bd32b588-07e4-4292-a5c4-db05a56ab94c", // Fairtrade
      "88e4b460-9a09-4118-bb46-a7950c8e7459" // V-Label
    ],
    locales: {
      en: {
        name: "Willow organic vegan super chocolate spread",
        price: "5.99",
        currency: "EUR",
        description: "Organic vegan chocolate spread with coconut palm sugar, palm oil free",
        quantities: ["300 g"],
        ingredients: "33% raw cocoa powder, vegetable oil (colza, coconut), coconut palm sugar, vanilla extract, salt, emulgator sun flower lecithin.",
        packagings: ["glass", "metal"],
        categories: ["Spread", "Breakfast", "Chocolate cream"],
        imageUrls: ["images/Willow_organic_vegan_super_chocolate_spread.jpg"],
        urls: ["https://willoworganic.com/products/vegan/"]
      }
    }
  }
  // {
  //   docType: "product",
  //   id: "418ff85c-ae31-448f-aebd-d77dd966a514",
  //   createdBy: "openfoodfacts-contributors",
  //   // "createdBy": "openfoodfactscontributorsgobbledigobbjabberdiwock",
  //   createdAt: "2016-02-10 16:58:18Z",
  //   status: "ACTIVE",
  //   updatedBy: "",
  //   updatedAt: "",
  //   supersedes: "",
  //   supersededBy: "",
  //   changeReason: "",
  //   score: {
  //     environment: -53,
  //     climate: -62,
  //     society: 20,
  //     health: -40,
  //     animalWelfare: 8,
  //     economy: 24
  //   },
  //   gtin: "7612100017487",
  //   producers: ["afd05a40-4ed6-4ae5-8120-eb7daebc336c"],
  //   containedProducts: [],
  //   labels: ["42c2f586-a893-485f-8995-8639446bb6b8"],
  //   locales: {
  //     en: {
  //       name: "The REAL Hot Chocolate",
  //       price: "3.99",
  //       currency: "EUR",
  //       description: "instant cocoa drink powder",
  //       quantities: ["400 g"],
  //       ingredients: "sugar, cocoa powder, milk powder, vanilla extract, salt",
  //       packagings: ["sachet"],
  //       categories: ["beverages", "sweets"],
  //       imageUrls: ["images/REAL_original_400g.jpg"],
  //       urls: [""]
  //     }
  //   }
  // },
  // {
  //   docType: "product",
  //   id: "cc536300-9bc9-465a-9df9-0b84b4f99050",
  //   createdBy: "odoc",
  //   createdAt: "2017-08-07 19:21:34Z",
  //   status: "ACTIVE",
  //   updatedBy: "",
  //   updatedAt: "",
  //   supersedes: "",
  //   supersededBy: "",
  //   changeReason: "",
  //   score: {
  //     environment: -34,
  //     climate: -46,
  //     society: 23,
  //     health: -43,
  //     animalWelfare: 8,
  //     economy: 29
  //   },
  //   gtin: "7612100054826",
  //   producers: ["afd05a40-4ed6-4ae5-8120-eb7daebc336c"],
  //   containedProducts: [],
  //   labels: ["42c2f586-a893-485f-8995-8639446bb6b8"],
  //   locales: {
  //     en: {
  //       name: "The REAL Hot Chocolate EXTRA",
  //       price: "4.69",
  //       currency: "EUR",
  //       description: "instant cocoa drink powder with added vitamins",
  //       quantities: ["500 g"],
  //       ingredients: "sugar, cocoa powder, milk powder, vitamins (A, E, B1, B2, C), vanilla extract, salt",
  //       packagings: ["plastic bag"],
  //       categories: ["beverages", "sweets"],
  //       imageUrls: ["images/REAL_500g.jpg"],
  //       urls: [""]
  //     }
  //   }
  // }
];